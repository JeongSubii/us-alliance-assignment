...
- 성능 관리 전략
	- 페이징 처리
	    : 대량 데이터 조회 시 slice로 페이징(네트워크 전송량만 줄임), 필요시 DB 전환 고려
	    (데이터가 적을 때는 실용적이지만, 데이터가 많아지면 반드시 DBMS로 전환하거나, Stream(stream-json, JSONStream 라이브러리) 등 다른 구조로 리팩터링)
	- 비동기/await 활용
	    : 모든 I/O 작업은 await로 논블로킹 처리
	- 에러 로깅
	    : 실패 시 로그 파일 기록 및 콘솔 경고로 장애 조기 감지
	- common 폴더
	    : 공통으로 쓰이는 dto, enum, interface 공통(common) 폴더에서 관리
